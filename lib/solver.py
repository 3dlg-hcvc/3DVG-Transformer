...
class Solver():
        ...
        for epoch_id in range(epoch):
            ...
            if epoch_id % 10 == 0:
                ...
                with torch.no_grad():
                    print("evaluating...")
                    # val
                    self._feed(self.dataloader["val"], "val", epoch_id, eval=True)
                    # self._dump_log("val")
    ...
    def _feed(self, dataloader, phase, epoch_id, eval=False):
        # switch mode
    ...
    #     mean_est_val_time = np.mean([forward for forward in forward_time])
